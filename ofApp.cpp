...
void ofApp::update() {
	...
	for (auto& location : base_location) {
		...
		for (int radius = 50; radius <= 300; radius += 15) {

			this->setRingToMesh(this->face, this->frame, location, noise_location, radius, 10, face_color);
		}
	}
}
...
void ofApp::setRingToMesh(ofMesh& face_target, ofMesh& frame_target, glm::vec3 location, glm::vec3 noise_location, float radius, float height, ofColor face_color) {
	...
	for (int deg = 0; deg < 360; deg += 90) {
		...
		vertices.push_back(glm::vec3(radius * cos((deg + 90) * DEG_TO_RAD), radius * sin((deg + 90) * DEG_TO_RAD), height * -0.5));
		vertices.push_back(glm::vec3(radius * cos((deg + 90) * DEG_TO_RAD), radius * sin((deg + 90) * DEG_TO_RAD), height * 0.5));
		vertices.push_back(glm::vec3(radius * cos(deg * DEG_TO_RAD), radius * sin(deg * DEG_TO_RAD), height * 0.5));
		...
		for (auto& vertex : vertices) {
			...
			auto noise_value_x = ofNoise(
				noise_location.x, radius * 0.0008 - ofGetFrameNum() * 0.0035);
			auto noise_value_y = ofNoise(
				noise_location.y, radius * 0.0008 - ofGetFrameNum() * 0.0035);
			auto noise_value_z = ofNoise(
				noise_location.z, radius * 0.0008 - ofGetFrameNum() * 0.0035);

			auto rotation_x = glm::rotate(glm::mat4(), ofMap(noise_value_x, 0, 1, -PI * 2, PI * 2), glm::vec3(1, 0, 0));
			auto rotation_y = glm::rotate(glm::mat4(), ofMap(noise_value_y, 0, 1, -PI * 2, PI * 2), glm::vec3(0, 1, 0));
			auto rotation_z = glm::rotate(glm::mat4(), ofMap(noise_value_z, 0, 1, -PI * 2, PI * 2), glm::vec3(0, 0, 1));

			vertex = glm::vec4(vertex, 0) * rotation_z + glm::vec4(location, 0);
		}
